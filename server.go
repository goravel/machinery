...
import (
	...
	backendsiface "github.com/goravel/machinery/backends/iface"
	...
	lockiface "github.com/goravel/machinery/locks/iface"
...
func (server *Server) RegisterPeriodicTask(spec, name string, signature *tasks.Signature) error {
	...
	f := func() {
		...
		if err != nil {
			log.ERROR("periodic task failed. task name is: %s. error is %s", name, err.Error())
		}
	}
